...
the bytes and the digest come out identical to upstream. `once --missing` skips
the comparison entirely for tags the target already has, which makes a run with
nothing to do finish in about a second.
...
- **Copying is idempotent**, so running the workflow when in doubt is free.
...
./regsync --config /tmp/local.yaml once --missing
```

The same rewrite aims a fork at its own GHCR namespace, if you want to exercise
the real thing before touching `k3s-io`:

```sh
sed 's|ghcr\.io/k3s-io|ghcr.io/your-user|' regsync.yaml > /tmp/fork.yaml
```

If you are pulling Docker Hub anonymously and hit `TOOMANYREQUESTS`, just run it
again — `--missing` skips what was already copied. Adding a `docker.io` entry to
`creds` with a Docker Hub login raises the limit, but at three tags from Docker
Hub it has not been needed.
